...
describe('dataProvider', () => {
  ...
  it('getList handles pagination params', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('range=%5B20%2C39%5D'));
  });
  ...
  it('getOne calls API with correct category param', async () => {
    ...
    expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('category=watches'));
  });
  ...
  it('create sends correct body', async () => {
    ...
    const result = await dataProvider.create('goods', { data: { name: 'New Watch' } });
  ...
  it('delete maps accessories to boxes', async () => {
    ...
    await dataProvider.delete('goods', { previousData: { id: '1', category: 'accessories' } } as any);
    expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('category=boxes'));
  });

  it('deleteMany joins ids correctly in URL', async () => {
    mockedAxios.get.mockResolvedValue({ data: [] });
    await dataProvider.deleteMany('goods', { ids: ['1', '2'] });
    expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('ids=%5B%221%22%2C%222%22%5D'));
  });

  it('should handle API network error gracefully', async () => {
    mockedAxios.get.mockRejectedValue(new Error('Network Error'));
    await expect(dataProvider.getList('goods', {})).rejects.toThrow('Network Error');
  });
});