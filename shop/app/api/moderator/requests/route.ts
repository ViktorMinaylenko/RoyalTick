...
export async function GET(req: Request) {
    try {
        ...
        const requests = await db.collection('chats').find({
            moderatorRequested: true,
            moderatorId: null,
        }).sort({ createdAt: -1 }).toArray()

        const myChats = await db.collection('chats').find({
            moderatorId: user._id,
        }).sort({ createdAt: -1 }).toArray()

        return NextResponse.json({ status: 200, requests, myChats }, corsHeaders)
    } catch (error) {
