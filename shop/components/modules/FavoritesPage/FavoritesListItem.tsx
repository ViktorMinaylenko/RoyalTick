...
const FavoritesListItem = ({ item }: { item: IFavoriteItem }) => {
  ...
  const addToCart = () => {
    const cartItem = {
      ...
      _id: item.productId,
      images: [item.image],
      characteristics: { vendorCode: item.vendorCode },
    }
  ...
  const sizeLabels = {
    straps: translations[lang].catalog.strap_size,
    watches: translations[lang].catalog.case_size,
  }

  const sizeLabel = sizeLabels[item.category as keyof typeof sizeLabels]
    ...
    <>
      ...
      <p className={styles.favorites__list__item__info}>
        ...
        <span className={styles.favorites__list__item__info__size}>
          {item.size.length ? `${sizeLabel}: ${item.size.toUpperCase()}` : ''}
        </span>
